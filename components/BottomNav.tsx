...
const BottomNav: React.FC = () => {
  ...
  const navClass = ({ isActive }: { isActive: boolean }) =>
    `flex flex-col items-center justify-center w-full h-full space-y-1 ${isActive ? 'text-brand-green' : 'text-gray-400'}`;
      ...
      <div className="grid grid-cols-5 h-full max-w-md mx-auto">
        ...
        <NavLink to="/cart" className={navClass}>
          <div className="relative">
            <ShoppingBag size={22} />
            {cartCount > 0 && (
              <span className="absolute -top-2 -right-2 bg-brand-green text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">
                {cartCount}
              </span>
            )}
          </div>
